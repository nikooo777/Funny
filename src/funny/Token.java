...
class Token
{
	static enum Type
	{
		...
		// Except for case, the spelling of the enums and the reserved words
		// shall be the same.
		// They are put in the reserved map by the class initializer.
		// The Bob_* and Eob_* marker names have been chosen to reduce the risk
		// of clashes with real-world reserved words.
	...
	static Token id(String name)
	{
		...
		return token != null ? new Token(token.type(), null, null) : new Token(Type.Id, null, name);
	}
	...
	public String toString()
	{
		...
		sb.append(Token.class.getSimpleName()).append(" [type=").append(type());
		if (name() != null)
