...
class Compiler {
    ...
    private boolean isInFirstOfAssignment() {
        ...
        case Print: case Println: return true;
        default: return false;
        }
    }
    ...
    private static Node combineWithOp(String id, Type type, Node expr) {
        switch (type) {
        case Becomes: return expr;
        case PlusBecomes: return new BinaryNode(Type.Plus, new GetVarNode(id), expr);
        case MinusBecomes: return new BinaryNode(Type.Minus, new GetVarNode(id), expr);
        case TimesBecomes: return new BinaryNode(Type.Times, new GetVarNode(id), expr);
        case DivBecomes: return new BinaryNode(Type.Div, new GetVarNode(id), expr);
        case ModBecomes: return new BinaryNode(Type.Mod, new GetVarNode(id), expr);
...
//    }
    ...
    private Node primary(Scope scope) throws IOException {
        switch (type()) {
        case Num: return num();
        case False: case True: return bool();
        case Nil: return nil();
        case String: return string();
        case Id: return getId(scope);
        case OpenBrace: return function(scope);
        case OpenParen: return subsequence(scope);
        case If: case IfNot: return cond(scope);
        case While: case WhileNot: return loop(scope);
        case Print: case Println: return print(scope);
        default: throw new CompilerException("invalid expression (" + token() + ")");
        }
    }
